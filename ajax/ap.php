...
if (!empty($file_upload['filename'])) {
    // Gunakan file asli tanpa konversi
    $video_file_full_path = $file_upload['filename'];
    
    $data['video_url'] = $pt->config->site_url . '/' . $video_file_full_path;
    $data['status'] = 200;
    $data['message'] = 'Video uploaded and ready to play without conversion.';
}
else {
    $data['status'] = 400;
