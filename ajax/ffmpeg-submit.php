...
if (!empty($_POST['privacy']) && $_POST['privacy'] == 3 && (empty($_POST['date']) || empty($_POST['hour']) || !in_array($_POST['hour'], array_keys($pt->config->hours)))) {
    ...
    if (empty($error)) {
        ...
        if ($pt->config->shorts_system == 'on' && !empty($_POST['is_short']) && $_POST['is_short'] == 'yes') {
            ...
            $video_path = $full_dir . $_POST['video-location'];
if (file_exists($video_path)) {
    $data['video_url'] = $pt->config->site_url . '/' . $_POST['video-location'];
    $data['status'] = 200;
    $data['message'] = 'Video available without conversion.';
} else {
    $data['status'] = 400;
    $data['message'] = 'Video not found at specified location.';
}
 @unlink($_POST['video-location']);
        ...
        if ($insert) {
            ...
            if ($processing_count < $pt->config->queue_count ||  $pt->config->queue_count == 0) {
                ...
                // gif Video
// Skip GIF generation since ffmpeg is disabled
if ($pt->config->gif_system == 'on' && empty($gif_video) && empty($_POST['is_movie'])) {
    // Optional: Set GIF as null or placeholder
    $db->where('id', $insert);
